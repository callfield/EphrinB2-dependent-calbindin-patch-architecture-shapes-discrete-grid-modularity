...
% The correlation is reported, but for statistical comparisons, the correlation coefficient was z-transformed to standard units (Bostock et al., 1991).
% https://www.jneurosci.org/content/28/44/11250#sec-2

% calculate percentage per running field
% “the proportion of active pixels.” It is the number of pixels in which one or more action potentials was detected divided by the total number of pixels the rat visited.
% https://www.jneurosci.org/content/28/44/11250#sec-2
...
function [fig_boot, HalvesCorr_Pearson_even, HalvesCorr_speed_Pearson_even, HalvesCorr_Pearson_Boot, CI_rateMap, CI_rateMap_90, R_boot_mean_rateMap, ...
    ...
    func_BootstrapRatemap_Shuffle_260204(thr_sp, SpeedResults, sigma, kernelSize, mice_ind, Day, i_Cell, AnimalTrack, spikeFrames, spikePos, binSize_cm)
...
edges = -1:0.02:1;
subplot(2,2,1)
% histogram(HalvesCorr_Pearson_Boot, 20,'FaceColor',[0.6 0.6 0.9]);
...
% plot([CI_rateMap(2) CI_rateMap(2)], yl, 'r--', 'LineWidth',2); % CI upper
hold off;
...
%     v_valid = v(validIdx);
%     r_valid = r(validIdx);
%     if numel(v_valid) >= 3  % ← 有効データが3点以上なら計算
%         f = fitLogistic(v_valid, r_valid);
%         slopes = f.k;  % slope
%     else
%         slopes = NaN;  % データ不足なら NaN
%     end
...
% rMap_thr_halve = cell(2,1);
% spikeMap_Rawcount_sp_halve = cell(2,1);
% numEvent_sp_halve = cell(2,1);
% spikeMap_count_thr_sp_halve = cell(2,1);
% runSpkIdx_sp_halve = cell(2,1);
% neuronPos_sp_halve = cell(2,1);
% occMapSmooth_sp_halve = cell(2,1);
% occSmooth_thr_sp_halve = cell(2,1);
for i = 1:2
    ...
    % % --- rate map  ---
    % [rateMap_halve, rateMap_thr_halve, ~, ~, ~, occMapSmooth_halve, occSmooth_thr_halve, spikeMap_Rawcount_halve, spikeMap_count_thr_halve] = ...
    %     func_computeSpatialRateMap_250915(mousePos_run_halve, neuronPos_halve, binSize_cm, VideoSampling, kernelSize, sigma, AnimalTrack, occThresh_reliability,...
    %     'minSpikes', 1, 'radius', 3, 'minNeighborEvent', 2);
    %
    % % --- save variables ---
    % % rateMap_sp_halve{i}         = rateMap_halve;
    % rMap_thr_halve{i}         = rateMap_thr_halve;
    % % spikeMap_Rawcount_sp_halve{i} = spikeMap_Rawcount_halve;
    % % numEvent_sp_halve{i}         = sum(spikeMap_Rawcount_halve(:));
    % % spikeMap_count_thr_sp_halve{i} = spikeMap_count_thr_halve;
    % % runSpkIdx_sp_halve{i}        = runSpkIdx_halve;
    % % neuronPos_sp_halve{i}        = neuronPos_halve;
    % % occMapSmooth_sp_halve{i}          = occMapSmooth_halve;
    % % occSmooth_thr_sp_halve{i}         = occSmooth_thr_halve;
...
% ★ 効果量マーク決定（Cliff’s δ基準: small=0.147, medium=0.33, large=0.47）★
...
% --- 表示用テキスト作成 ---
% txt = sprintf('p = %.3g  (%s),  Cliff\\delta = %.3f  (%s)', p_rank, sigMark, Cliff_delta, effMark);
...
% pos = [pos; AreanaxEdges(1), AreanayEdges(1); AreanaxEdges(end), AreanayEdges(end)];
...
% % xEdges = [0:1:xEdges(1), xEdges(2:end-1)', xEdges(end):1:AreanaxEdges(end)+1];
% % yEdges = [0:1:yEdges(1), yEdges(2:end-1)', yEdges(end):1:AreanayEdges(end)+1];
% xEdges = sort(xEdges);
% yEdges = sort(yEdges);
