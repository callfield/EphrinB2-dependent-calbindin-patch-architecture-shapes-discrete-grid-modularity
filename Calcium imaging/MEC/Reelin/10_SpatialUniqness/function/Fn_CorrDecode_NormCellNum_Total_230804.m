function [MEAN, TOTAL, MAX]=Fn_CorrDecode_NormCellNum_Total_230804(DIR)
close all;
% Mod = wt_Mod{s,t};
            ...
            for BaseY=1:50
                for y=1:50
                    tmp=corr(tmpGSrate_map(CELLSET,:,BaseY), ...
                        TotalRatemap(CELLSET,:,y), 'rows','pairwise');
                    for x=1:50
...
strcat("Smallest mean error from 50 cell/",num2str(num_SHUFFLE*num_SHUFFLE2)," shuffle")},...
    'FontSize',14);
exportgraphics(gcf,strcat(CDir, "\MeanError\MeanError averagedFrameSet bestCellSet" ,samplename,".jpg"))
        clf
...
strcat("Smallest total error from 50 cell/",num2str(num_SHUFFLE*num_SHUFFLE2)," shuffle")},...
    'FontSize',14);
exportgraphics(gcf,strcat(CDir, "\TotalError\TotalError averagedFrameSet bestCellSet" ,samplename,".jpg"))
        clf
...
strcat("Smallest Max error from 50 cell/",num2str(num_SHUFFLE*num_SHUFFLE2)," shuffle")},...
    'FontSize',14);
exportgraphics(gcf,strcat(CDir, "\MaxError\TotalError averagedFrameSet bestCellSet" ,samplename,".jpg"))
        clf
