function [posterior] = bayesian_decode_quantitate(spikes,test_start,test_end,alpha,beta)
%{
...
M = length(test_start);    % Number of decoding steps.
...
%    bin_edges = reshape([test_start;test_end],1,[]);
    bin_edges = reshape([test_start test_end(end)],1,[]);
...
%    pop_vec(i,:) = counts_per_bin(1:2:end);
     pop_vec(i,:) = counts_per_bin;

%    for tmp=1:1:length(test_start)
%         pop_vec(i,tmp)=length(find(ts>=test_start(tmp)&ts<test_end(tmp)));
%    end
end

tau = test_end - test_start; % Decoding timesteps.
...
% c = sum(reshape(posterior,M,prod(occ_size)),2,'omitnan');
