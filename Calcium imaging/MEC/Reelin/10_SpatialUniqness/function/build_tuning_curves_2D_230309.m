function [coords,varargout] =  build_tuning_curves_2D_230309(spikes,X,t,...
    sample_rate,train_use,bin_size,sigma,f_base,min_t_occ)
%{
...
a distribution over firing rate (using a gamma-distributed prior).
	...
	Args:
        ...
        sample_rate: sample rate of ground-truth data.>> scale change from 
        second to Hz (Original was second, but 1/6 has problem)
...
for i = 1:N
    ...
    bins = bin_size(i)/2:bin_size(i):100-bin_size(i)/2;
    grid_vectors{i} = bins;
...
t_occ = convn(t_occ,G,'same'); % this is bad, smooth only one axis
% bin=bin_size(1);
% if bin <10
%     Diskfilter = fspecial('disk',ceil((5-bin/2)/bin)); %Circular averaging filter, radium 5cm
% %     Diskfilter(Diskfilter>0)=1; % change to Circular filter to increase point
%     increase_t_occ = imfilter(t_occ,Diskfilter,'replicate','same'); 
%     t_occ = imgaussfilt(increase_t_occ,sigma);
% else
%     t_occ = imgaussfilt(t_occ,sigma);
% end
...
% Build the tuning curve for each unit(cell).
for i = 1:K
    ts = spikes{i};
...
%     ts = ts(ts_use);
    ts = intersect(ts,train_use/sample_rate);
    ...
    % Smooth spike counts.
        n = convn(n,G,'same'); % this is bad, smooth only x-axis
    % increase fireing point to neighbor
%     bin=bin_size(1);
%     Diskfilter = fspecial('disk',ceil((5-bin/2)/bin)); %Circular averaging filter, radium 5cm
%     Diskfilter(Diskfilter>0)=1; % change to Circular filter to increase point
%     
...
%     
%     if bin <10
%         increase_n = imfilter(n,Diskfilter,'replicate','same'); 
%         n = imgaussfilt(increase_n,sigma);
%     else
%         n = imgaussfilt(n,sigma);
%     end   
