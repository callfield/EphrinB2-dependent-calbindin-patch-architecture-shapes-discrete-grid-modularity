function [lineOut, fillOut] = stdshade_sem_nan2zero(amatrix,alpha,acolor,F,smth)
...
amatrix(amatrix==0)=NaN;% convert zero to nan before mean

amean = nanmean(amatrix,1); %get man over first dimension

amean(isnan(amean)) = 0;% convert nan to zero
...
astd(isnan(astd)) = 0; % convert nan to zero
