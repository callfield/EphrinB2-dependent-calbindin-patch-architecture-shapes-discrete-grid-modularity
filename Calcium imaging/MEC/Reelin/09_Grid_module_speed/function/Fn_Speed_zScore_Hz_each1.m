function [ZSpeedHz, SpeedHz]=Fn_Speed_zScore_Hz_each1(DIR)
close all;


load(strcat(DIR,"\ST_dF_grid_aut_data.mat"))
...
Vframe=cell(30,1);
for v=1:1:60
    % if v=10cm/s, use frame with 6-14
    Vframe{v}=find(V<=v+4 & V>(v-6));
end
%%

% make reference z score using whole frame
TestFrameNum=1000;
NumRepeat=10000; 
% prepare shufflefrom 0<v<=5
...
SpeedHz=nan(60,numCells);
ZSpeedHz=nan(60,numCells);
for c=1:1:numCells 
...
%% 
    ...
    for v=1:1:60
        if length(Vframe{v})>=50;
            tmp=intersect(FireFrame{c},Vframe{v});
            SpeedHz(v,c)=caFr*length(tmp)/length(Vframe{v});
            ZSpeedHz(v,c)=(SpeedHz(v,c)-MEAN)/SD;
        end
    end
