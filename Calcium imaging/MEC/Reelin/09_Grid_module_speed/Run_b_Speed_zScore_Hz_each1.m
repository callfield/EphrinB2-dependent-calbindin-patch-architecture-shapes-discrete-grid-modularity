...
for s=1:5
    for t=1:3
        samplename=strcat(SampleName{1,s}, " T",num2str(t));
         ...
         [gp1_ZSpeedHz{s,t}, gp1_SpeedHz{s,t}]=Fn_Speed_zScore_Hz_each1(DIR);
        ...
        for dv=1:1:3
            ...
            if isempty(tmpcell)==0
                ...
                tmp=gp1_ZSpeedHz{s,t}(1:20,tmpcell);
        ...
        % exportgraphics(gcf,strcat(CDir,"\ZScored_all_trial_each1\ZScored_DVspeed_",samplename,".jpg"))
        exportgraphics(gcf,strcat(CDir,"\ZScored_all_trial_each1\ZScored_DVspeed_",samplename,".pdf"))
clf
    end
end


save("ZSpeedHz_each1.mat","gp1_ZSpeedHz","gp1_SpeedHz",'-append')
...
for s=1:7
    for t=1:3
        ...
        [gp2_ZSpeedHz{s,t}, gp2_SpeedHz{s,t}]=Fn_Speed_zScore_Hz_each1(DIR);
        ...
        for dv=1:1:3
            ...
            if isempty(tmpcell)==0
                ...
                tmp=gp2_ZSpeedHz{s,t}(1:20,tmpcell);
                tmp(tmp==0)=NaN;% convert zero to nan before mean
...
save("ZSpeedHz_each1.mat","gp2_ZSpeedHz","gp2_SpeedHz",'-append')
