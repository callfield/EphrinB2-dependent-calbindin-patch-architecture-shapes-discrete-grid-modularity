...
%% Grid field scatter with KSD regression slope
% Keep grid cells with positive grid scale, then analyze column 5.
gp1Parts = cell(7, 3);
for s = 1:7
    for t = 1:3
...
data1 = gp1;
data2 = gp2;
peaks1 = ksdnorm200_field(data1);
peaks2 = ksdnorm200_field(data2);
...
plot_GridFieldVsLocation_KSDRegression_2(data1, peaks1, xlims, ylims);
exportgraphics(gcf, fullfile(scriptDir, "G_field_scatter_with_slope_group1_all_trial.emf"), "ContentType", "vector")
close all

figure('Position', [100, 100, 250, 500]);
plot_GridFieldVsLocation_KSDRegression_2(data2, peaks2, xlims, ylims);
exportgraphics(gcf, fullfile(scriptDir, "G_field_scatter_with_slope_group2_all_trial.emf"), "ContentType", "vector")
close all

%% Region-adjusted Grid field distribution
% data1: group 1
% data2: group 2
% Column 1: sampling region, e.g. DV position
% Column 5: Grid field width
...
H_adj = p_perm < 0.05;
fprintf('Region-adjusted permutation: H = %d, D = %.4f, p = %.4f\n', H_adj, D_adj, p_perm)
...
%% Export
outFile = fullfile(scriptDir, "G_field_regionAdjusted_all_trial.emf");
exportgraphics(fig, outFile, "ContentType", "vector");

close

[H, p, ksstat] = kstest2(gs1, gs2);
fprintf('Unadjusted KS test: H = %d, p = %.4f, KS stat = %.4f\n', H, p, ksstat)
